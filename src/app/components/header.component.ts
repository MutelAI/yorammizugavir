    ...
    >
      <div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <a href="#hero" class="flex items-center gap-2 group" (click)="scrollTo('hero', $event)">
          ...
          <div>
            <div [class]="'font-bold text-lg leading-tight transition-colors ' + (scrolled() ? 'text-emerald-700' : 'text-white')">
        ...
        <nav class="hidden md:flex items-center gap-6">
          @for (item of navItems; track item.key) {
            <a
              [href]="'#' + item.anchor"
              [class]="'text-sm font-medium transition-colors hover:text-emerald-400 ' + (scrolled() ? 'text-gray-700' : 'text-white')"
        ...
        <div class="flex items-center gap-3">
          ...
          <a
            ...
            [class]="'hidden md:flex items-center gap-1 text-sm font-semibold px-3 py-1.5 rounded-full transition-all ' + (scrolled() ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-white/20 text-white hover:bg-white/30')"
          >
          ...
          @if (i18n.showLangToggle()) {
            <button
              ...
              [class]="'text-sm font-medium px-3 py-1.5 rounded-full border transition-all ' + (scrolled() ? 'border-emerald-600 text-emerald-600 hover:bg-emerald-50' : 'border-white/60 text-white hover:bg-white/10')"
            >
              {{ i18n.t()('lang_toggle') }}
            </button>
          }
        ...
        <div class="md:hidden bg-white shadow-lg border-t border-gray-100 px-4 py-4">
          <nav class="flex flex-col gap-3">
            @for (item of navItems; track item.key) {
              <a
                [href]="'#' + item.anchor"
                class="text-gray-700 font-medium py-2 border-b border-gray-100 hover:text-emerald-600 transition-colors"
                (click)="scrollTo(item.anchor, $event); mobileOpen.set(false)"
              >{{ i18n.t()(item.key) }}</a>
            }
            <a
              [href]="'tel:' + biz.business()?.phone"
              class="mt-2 bg-emerald-600 text-white text-center py-3 rounded-lg font-semibold"
